...
public class Falling : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        //Adds spawn points. Can add or delete as many as you want
    ...
    private List<GameObject> Algorithm()
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            for (int j = 0; j < Ingredients.Count; j++) // Searches every ingredient in Ingredients to see if it matches the ingredient in food
            {
                do
                {
                    if (Ingredients[j] == food[randFood] ) // if it does, then sets dont add to true
                    {
    ...
    void Spawn()
    {
        //change according to number of spawn points
    ...
    {
        foreach(GameObject food in AlgorithmFood)
        {
            Debug.Log(food);
        }
    }
